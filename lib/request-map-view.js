...
var mapContextMenuTemplate = require('./templates/map-context-menu.handlebars')
...
var RequestMapView = Backbone.View.extend({
  ...
  render: function () {
    ...
    if (from && !isNaN(from[0]) && !isNaN(from[1])) {
      ...
      this.startMarker.bindTooltip('<strong>Start</strong>')
    ...
    if (to && !isNaN(to[0]) && !isNaN(to[1])) {
      ...
      this.endMarker.bindTooltip('<strong>End</strong>')
